...
void las_args_free(void) {
  void *argtable[] = las_argtable;
  arg_freetable(argtable, sizeof(argtable) / sizeof(argtable[0]));
}

void las_args_parse(int argc, char **argv) {
...
int main(int argc, char **argv) {
  las_args_parse(argc, argv);
  
  // map args to cfg here 
  struct las_config cfg;
  memset(&cfg, 0, sizeof(cfg));

  cfg.verbose = verb->count > 0;

  int res = las_main(&cfg);
